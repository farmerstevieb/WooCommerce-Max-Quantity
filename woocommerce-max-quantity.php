...
if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins'))) || in_array('woocommerce/woocommerce.php', array_keys(apply_filters('active_plugins', get_site_option('active_sitewide_plugins'))))) {
    ...
    /**
     * do we need to reset the qty in the cart to Max
     *
     * @param mixed $product_id ID of the product in question
     * @param string $cart_item_key The cart key for this item in case of Updating cart
     *
    ...
    function isa_wc_max_qty_add_to_cart_validation($passed, $product_id, $quantity, $variation_id = '', $variations = '')
    {
        ...
        if (!empty($already_in_cart)) {
            ...
            if (($already_in_cart + $quantity) > $new_max) {
                ...
                wc_add_notice(apply_filters('isa_wc_max_qty_error_message_already_had', sprintf(__('You can only add a maximum of %1$s %2$s\'s to %3$s. You already have %4$s.', 'woocommerce-max-quantity'),
                    $new_max,
                    $product_title,
                    '<a href="' . esc_url(wc_get_cart_url()) . '">' . __('your Basket', 'woocommerce-max-quantity') . '</a>',
                    $already_in_cart),
                    $new_max,
                    $already_in_cart),
                    'error');
    ...
    /**
     * Display the product's "Max Quantity Per Purchase" field in the Product Data metabox
     * @since 1.4
     */
